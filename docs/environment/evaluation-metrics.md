...
密度 MAE は 2 つの尺度で測る。**AC-03 の判定値は 3.1 の密度領域 MAE**、3.2 の画素領域 MAE は補助指標とする（OI-003）。
...
- **目標密度 `D_target[c]`**: 目標画像を各セル矩形へ面積平均で縮約し、2章の `d = 1 − Y` を取る（連続値 0–1）。
  ...
  （量子化誤差を評価へ持ち込まないため）。両尺度で同じ `D_target` を使う。
- **セル MAE**（両尺度に共通の式）:
...
### 3.1 密度領域 MAE（AC-03 の判定値）

- **達成密度 `D_achieved[c]`**: そのセルへ置いた文字の黒画素率を、密度辞書内の最小〜最大で 0–1 へ正規化した値
  （`optpoet.text.evaluate.density_table`）。

  ```
  D_achieved[c] = (ratio(char_c) − min_ratio) / (max_ratio − min_ratio)
  ```

  `max_ratio = min_ratio` の辞書（1 文字辞書など）では写像を定数 0 とする。
- **理由**: 文字の黒画素率は基準フォント・基準セルで 0.000–0.153 に収まり、目標密度が使う 0–1 の全域に届かない。
  正規化しない差分では、どの文字を選んでも暗部の誤差が残り、文字選択の良否が MAE に表れない
  （[phase-1-gate.md](phase-1-gate.md) 3.2 の実測）。正規化後は探索の目的関数と判定値が一致する。
- **正規化条件の固定**: 正規化は辞書に依存するため、測定ごとに `dictionary_id`、フォント（family と hash）、
  描画設定（`pixel_size` / `cell_width` / `cell_height`）、`min_ratio` / `max_ratio` を記録する。
  これらが異なる測定値を同列に比較しない。
- **AC-03 判定（視覚改善）**: 各画像で草稿配置と最適化後の MAE を測り、改善率を
  ...
  除外枚数を記録する。判定を課す完了ゲートは [P2-G01](../roadmap/phase-2.md)（OI-004）。
- **参考値**: 密度だけで自由に文字を選ぶ基準配置（[P1-034](../roadmap/phase-1.md)）の MAE を同時に測り、
  その配置構成での下限として併記する。
...
- 実画面の見え方に直結するため記録は続けるが、値域が目標密度と揃わないため合否には用いない。
  改善率は 3.1 と同じ式で求め、両尺度を併記する。
...
- 各作品で密度領域・画素領域それぞれの `MAE_draft` / `MAE_opt` / 改善率と、SSIM / エッジ一致率 / 重要領域 MAE を記録する。
- データセット全体の集計は密度領域の改善率の中央値（3.1）で AC-03 を判定する。他指標は中央値と分布を併記し単一値で断定しない。
- 密度領域の値には正規化条件（`dictionary_id` / フォント / 描画設定 / `min_ratio` / `max_ratio`）を併記する（3.1）。
...
- 密度領域(判定): MAE_draft <..>  MAE_opt <..>  改善率 <..>  基準配置MAE <..>
- 正規化条件: dictionary_id <..> / フォント <family> <hash> / 描画設定 <pixel_size>-<cell_w>x<cell_h> / ratio <min>–<max>
- 画素領域(補助): MAE_draft <..>  MAE_opt <..>  改善率 <..>
- SSIM(opt): <..>  エッジ一致率(opt): <..>
- 重要領域MAE: <.. / 該当なし>
