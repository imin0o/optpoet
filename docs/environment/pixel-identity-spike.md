...
これにより完了ゲート [P0-G02](../roadmap/phase-0.md)（基準フォントと描画エンジンの実測整合）は**暫定クローズ**とした（2026-07-25 判断）。本スパイクが確認したのは「描画方式が同一画素を保証すること」であり、この結論はフォントファイルに依存しない。基準フォントファイル確定（[P1-020](../roadmap/phase-1.md)）後に同一手順で再実測し、結果を本書へ追記する。
