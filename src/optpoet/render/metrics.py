...
- 画素領域の密度 MAE（3.2）: 生成結果 PNG をセルグリッドへ縮約して測る。離散化前の連続密度を
  使い、量子化誤差を評価へ持ち込まない。
- SSIM（4 章）: 生成結果 PNG の画素寸法を比較解像度とし、目標画像をそこへ揃える。
- エッジ一致率（5 章）: ガウシアンぼかし → Canny → Dice 係数。

本モジュールの指標は MVP ではすべて補助指標で、固定閾値の合否には使わない。AC-03 の判定値は
辞書正規化密度で測る MAE（`optpoet.text.evaluate` / evaluation-metrics.md 3.1、OI-003）。
"""
